...
describe("Intl.RelativeTimeFormat", function () {
        ...
        data: {
          en: {
            ...
            "quarter-short": {
              0: "this qtr.",
              1: "next qtr.",
              future: { one: "in {0} qtr", other: "in {0} qtr" },
              past: { one: "{0} qtr ago", other: "{0} qtr ago" },
              "-1": "last qtr."
            },
            "quarter-narrow": {
              0: "this qtr.",
  ...
  it("format should work for numeric auto", function () {
    // Since we rely on Intl.NumberFormat.formatToParts which is missing in some browsers we must skip this test in these browsers
    // See https://github.com/andyearnshaw/Intl.js/issues/242 for a discussion about partially polyfilling Intl APIs
  ...
  it("formatToParts should work for numeric auto", function () {
    // Since we rely on Intl.NumberFormat.formatToParts which is missing in some browsers we must skip this test in these browsers
    // See https://github.com/andyearnshaw/Intl.js/issues/242 for a discussion about partially polyfilling Intl APIs
    if (Intl.NumberFormat.prototype.formatToParts) {
      proclaim.deepEqual(
        new Intl.RelativeTimeFormat("en", { numeric: "auto" }).formatToParts(
          -1,
          "day"
        ),
        [
