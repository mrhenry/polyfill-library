...
describe('Intl.PluralRules', function () {
  ...
  it('should work for ordinal', function () {
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(0), 'other');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(1), 'one');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(2), 'two');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(3), 'few');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(-1), 'one');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(-2), 'two');
    proclaim.equal(new Intl.PluralRules('en', { type: 'ordinal' }).select(-3), 'few');
  });
