...
describe('Intl.DateTimeFormat.timeZone.golden', function () {
	it('works with short format', function () {
		...
		proclaim.equal(new Intl.DateTimeFormat('en', options).format(date).slice(-6), 'GMT+11');
	});

	it('works with long format', function () {
		var date = new Date(Date.UTC(2012, 11, 20, 3, 0, 0, 200));
		var options = {
			weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
			timeZone: 'Australia/Sydney',
		...
		proclaim.equal(new Intl.DateTimeFormat('en', options).format(date).slice(-32), 'Australian Eastern Daylight Time');
	});
});
