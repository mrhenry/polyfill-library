...
async function applyResolverToIdentifiers(featureSet, nameResolverFunction) {
	...
	while (processQueue.length > 0) {
		...
		resultItems.forEach(function(newName) {
			if (newName !== featureName) {
				...
				if (!featureSet[newName]) {
					featureSet[newName] = Object.assign({}, feature, properties);
					...
					['flags', 'aliasOf'].forEach(property => {
						for (const v of properties[property]) {
							if (!(property in featureSet[newName])) {
								featureSet[newName][property] = new Set();
							}
							featureSet[newName][property].add(v);
						}
					});
				}
			}
		...
		if (!resultItems.includes(featureName)) delete featureSet[featureName];
	}
