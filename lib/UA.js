		...
		if (aliases[this.ua.family]) {
			...
			// Map to different family with per-version mapping
				...
				for (let semverExpr in aliases[this.ua.family]) { // eslint-disable-line prefer-const
					if (satisfies(this.ua, semverExpr) && Array.isArray(aliases[this.ua.family][semverExpr])) {
...
UA.prototype.getVersion = function() {
	return `${this.ua.major}.${this.ua.minor}.0`;
};
function satisfies (ua, range) {
	return semver.satisfies((Number(ua.major) || 0) + '.' + (Number(ua.minor) || 0) + '.' + (Number(ua.patch) || 0), range);
};
UA.prototype.satisfies = function(range) {
	return (
		satisfies(this.ua, range) &&
		this.ua.family in baseLineVersions &&
		satisfies(this.ua, baseLineVersions[this.ua.family])
	);
};
...
UA.prototype.meetsBaseline = function() {
	return satisfies(this.ua, baseLineVersions[this.ua.family]);
};
