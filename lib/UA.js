		...
		if (aliases[this.ua.family]) {
			...
			// Map to different family with per-version mapping
				...
				for (let semverExpr in aliases[this.ua.family]) { // eslint-disable-line prefer-const
					if (semver.satisfies(this.getVersion(), semverExpr) && Array.isArray(aliases[this.ua.family][semverExpr])) {
...
UA.prototype.getVersion = function() {
	return `${(Number(this.ua.major) || 0)}.${(Number(this.ua.minor) || 0)}.0`;
};
UA.prototype.satisfies = function(range) {
	return (
		semver.satisfies(this.getVersion(), range) &&
		this.ua.family in baseLineVersions &&
		semver.satisfies(this.getVersion(), baseLineVersions[this.ua.family])
	);
};
...
UA.prototype.meetsBaseline = function() {
	return semver.satisfies(this.getVersion(), baseLineVersions[this.ua.family]);
};
