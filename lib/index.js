  ...
  for (const featureName of features) {
    ...
    if (alias) {
      ...
      for (const aliasedFeature of alias) {
        targetedFeatures[aliasedFeature] = Object.assign({}, feature, aliasProperties);
        features.add(aliasedFeature);
      }
    } else {
      ...
      if (meta) {
        ...
        const hasAlwaysFlagOverride = feature.flags.has("always");
        ...
        if (isBrowserMatch || hasAlwaysFlagOverride || unknownOverride) {
          targeted = true;
        }
      } else {
        targeted = true;
      ...
      if (targeted) {
        if (!targetedFeatures[featureName]) {
          targetedFeatures[featureName] = Object.assign({}, feature, properties);
        } else {
          ...
          ['flags', 'aliasOf', 'dependencyOf'].forEach(property => {
            for (const v of properties[property]) {
              if (!(property in targetedFeatures[featureName])) {
                targetedFeatures[featureName][property] = new Set();
              }
              targetedFeatures[featureName][property].add(v);
            }
          });
        }
        if (meta && meta.dependencies) {
          ...
          dependencyProperties.dependencyOf.add(featureName);
          for (const dep of deps) {
            targetedFeatures[dep] = Object.assign({}, feature, dependencyProperties);
            features.add(dep);
          }
        }
      } else {
