...
async function featureRequiresTesting(feature) {
	...
	const filesWhichChanged = execa.commandSync('git diff --name-only origin/main HEAD').stdout.split('\n');
	...
	const packageJsonDependenciesFromMaster = JSON.parse(execa.commandSync('git show origin/main:package.json').stdout).dependencies;
